...
public sealed class Timestamp : IEquatable<Timestamp>, IComparable<Timestamp>
{
    ...
    public Timestamp(long ticks)
    {
        ...
        object arg2 = minValue.Ticks;
        minValue = DateTime.MaxValue;
        throw new ArgumentOutOfRangeException("ticks", $"Ticks {arg} is not in range [{arg2}, {minValue.Ticks}]");
    }
    ...
    public int CompareTo([CanBeNull] Timestamp other)
    {
        if (!(other == null))
        {
            return Comparer<long>.Default.Compare(Ticks, other.Ticks);
        }
        return 1;
    }
    ...
    {
        long ticks = left.Ticks + right.Ticks;
    ...
    {
        DateTime minValue = DateTime.MinValue;
        MinValue = new Timestamp(minValue.Ticks);
        minValue = DateTime.MaxValue;
        MaxValue = new Timestamp(minValue.Ticks);
    }
}